...
已按用户研究方向把 8 个常用领域 skill 长期提回核心：`pydicom`、`imaging-data-commons`、`clinical-reports`、`histolab`、`pathml`、`pyhealth`、`nextflow`、`scanpy`。

结果：核心 35 → **43 个**，每会话描述开销 ~3,685 → **~4,681 token**（原 16,561，仍省约 72%）。其余 88 个领域 skill 留在 `skills-library/` 按需启用。
